...
public class FilePathUtils {
    ...
    public static final String getFilePathForDB(String... args) {
        StringBuilder path = new StringBuilder();
        for (int i = 0; i < (args.length - 1); i++) {
            path.append(args[i] + FILE_SEPARATOR + FILE_SEPARATOR);
        }
        path.append(args[args.length - 1]);
        return path.toString();
    }

    public static final String getFolderPathForDB(String... args) {
        StringBuilder path = new StringBuilder();
        for (String arg : args) {
            path.append(arg + FILE_SEPARATOR + FILE_SEPARATOR);
        }
        return path.toString();
    }
